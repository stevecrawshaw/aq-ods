...
banes_all_sites_tbl <- dt_sites_tbl |>
  ...
  mutate(
    ...
    tube_co_located_with_a_continuous_analyser = if_else(
      is.na(tube_co_located_with_a_continuous_analyser) &
        monitoring_technique == "Diffusion tube",
      "No",
      tube_co_located_with_a_continuous_analyser
    ),
    # These lines to account for BANES site where pumped BTX not used
    pollutants_monitored = if_else(
      site_id == "CM8",
      "NO2, PM10",
  ...
  mutate(
    monitor_type = if_else(
      str_detect(monitoring_technique, "Diffusion"),
      "Diffusion tube",
      "Continuous"
    )
  ) |>
...
banes_all_sites_tbl |>
  st_write("data/banes_aq_sites.geojson", driver = "GeoJSON", delete_dsn = TRUE)
...
dt_concs_am <- dt_table_a4_tbl |>
  ...
  mutate(across(.cols = starts_with("x"), .fns = as.double)) |>
  pivot_longer(
    cols = starts_with("x2"),
    names_to = "year",
...
write_rds(
  list(
    "banes_aq_concs_tbl" = banes_aq_concs_tbl,
    "banes_all_sites_tbl" = banes_all_sites_tbl
  ),
