...
dt_sites_tbl <- dt_sites_raw |>
  mutate(
    ...
    tube_co_located_with_a_continuous_analyser = str_detect(
...
cs_sites_tbl <- cs_sites_raw |>
  mutate(
    across(
      .cols = c(x_os_grid_ref_easting, y_os_grid_ref_northing),
      .fns = as.numeric
    ),
    aqma = get_clean_bracket(in_aqma_which_aqma),
    aqma_bool = get_aqma_bool(in_aqma_which_aqma),
    height_m = str_extract_all(inlet_height_m, "\\d\\.\\d") |>
      ...
      map(mean) |>
      unlist(),
    distance_to_relevant_exposure_m = na_if(
      distance_to_relevant_exposure_m_1,
      "N/A"
    ) |>
      as.double(),
  ...
  rename(
    distance_to_kerb_of_nearest_road_m = distance_to_kerb_of_nearest_road_m_2
  ) |>
  select(
    -c(inlet_height_m, in_aqma_which_aqma, distance_to_relevant_exposure_m_1)
  ) |>
  glimpse()
...
s_glos_all_sites_tbl <- dt_sites_tbl |>
  ...
  mutate(la_name = la_name, ladcd = ladcd) |>
  st_as_sf(
...
cs_concs_no2_am <- readxl::read_xlsx(
  southglos_path,
...
cs_concs_pm10_am <- readxl::read_xlsx(
  southglos_path,
...
cs_concs_pm25_am <- readxl::read_xlsx(
  southglos_path,
...
write_rds(
  list(
    "s_glos_aq_concs_tbl" = s_glos_aq_concs_tbl,
    "s_glos_all_sites_tbl" = s_glos_all_sites_tbl
  ),
