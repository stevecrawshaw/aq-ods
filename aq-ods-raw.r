...
get_raw_dt_la_lst <- function(la_list) {
  ...
  dt_step_2_inputs_tbl <-
    read_xlsx(dt_path, sheet = "STEP 2 - Diffusion Tube Inputs", skip = 11) |>
  ...
  dt_table_a2_tbl <-
    read_xlsx(dt_path, sheet = "Table A.2", skip = 8) |>
    clean_names(replace = c("\u00b5" = "u")) |>
    select(-matches("x\\d")) |>
    filter(!is.na(diffusion_tube_id))

  dt_a4_names_parameters <- read_xlsx(
    dt_path,
    sheet = "Table A.4",
    range = "C9:H9"
  ) |>
  ...
  dt_table_a4_tbl <-
    read_xlsx(dt_path, sheet = "Table A.4", skip = 10) |>
    select(1:11) |>
    set_names(dt_a4_names) |>
    filter(!is.na(diffusion_tube_id))

  dt_table_dtdes_tbl <- read_xlsx(dt_path, sheet = "DTDES Inputs", skip = 1) |>
    clean_names(replace = c("\u00b5" = "u")) |>
    select(-matches("x\\d")) |>
...
get_raw_cs_la_lst <- function(la_list) {
  ...
  cs_table_a1 <- read_xlsx(cs_path, sheet = "Table A.1", skip = 4) |>
    clean_names(replace = c("\u00b5" = "u"))

  # annual mean concentrations NO2
  cs_table_a3 <- read_xlsx(cs_path, sheet = "Table A.3", skip = 4) |>
    clean_names(replace = c("\u00b5" = "u"))

  # 1 hour means NO2

  cs_table_a5 <- read_xlsx(cs_path, sheet = "Table A.5", skip = 4) |>
    clean_names(replace = c("\u00b5" = "u")) |>
    glimpse()
  ...
  cs_table_a6 <- read_xlsx(cs_path, sheet = "Table A.6", skip = 4) |>
    clean_names(replace = c("\u00b5" = "u")) |>
    glimpse()
  ...
  cs_table_a7 <- read_xlsx(cs_path, sheet = "Table A.7", skip = 4) |>
    clean_names(replace = c("\u00b5" = "u")) |>
    glimpse()
  ...
  cs_table_a8 <- read_xlsx(cs_path, sheet = "Table A.8", skip = 4) |>
    clean_names(replace = c("\u00b5" = "u")) |>
    glimpse()
