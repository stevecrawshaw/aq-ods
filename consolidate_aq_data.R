...
bristol_aq_tbl <- read_rds("data/bristol_no2_annual_long.rds") |> 
  ...
  rename(x_os_grid_ref_easting  = x,
         y_os_grid_ref_northing   = y) |> 
  glimpse()
...
consolidated_no2_dt_aq_weca_tbl <- bind_rows(bristol_aq_tbl, banes_aq_tbl, sgc_aq_tbl) |>
  mutate(local_authority = fct_relevel(local_authority, "Bristol, City of", "Bath and North East Somerset", "South Gloucestershire")) |>
...
consolidated_no2_dt_aq_weca_tbl |> 
  st_write("data/consolidated_no2_dt_aq_weca.geojson",
           driver = "GeoJSON",
           delete_dsn = TRUE)
