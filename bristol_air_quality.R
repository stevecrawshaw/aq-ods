...
tubes_cleaner_tbl <- tubes_raw |> 
  mutate(site_id = if_else(str_detect(site_no, "_"),
                           str_sub(site_no, 1, 3),
                           site_no) |> 
           as.integer(),
         ...
         x = as.integer(x),
         y = as.integer(y)) |> 
  rename(conc_2023 = x2023_annualised_and_bias_adjusted_0_85_not_distance_adjusted) |> 
  glimpse()
...
annual_tube_data_bristol_long_tbl <- tubes_cleaner_tbl |> 
  ...
  pivot_longer(cols = matches("x2|conc_"),
               names_to = "year",
               values_to = "concentration") |>
  mutate(year = str_remove(year, "x|conc_") |> as.integer(),
...
st_write(tubes_zebra_out_sf,
         "data/bristol_no2_dt_2023.shp",
         append = FALSE)
