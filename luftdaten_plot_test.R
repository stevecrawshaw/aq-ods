...
field_filter_str_fnc <- function(
  field_name = "siteid",
  values_vec = c("203", "215")
) {
...
sts_sensors <- field_filter_str_fnc(
...
if (!file.exists("data/met_raw.rds")) {
  met_raw <- getODSExport(
    select_str = "date_time, temp, ws, wd, rh",
    where_str = "",
    date_col = "date_time",
    dateon = date_on,
    dateoff = date_off,
...
nest_prep_fnc <- function(data) {
  data %>%
    ...
    # head(5) %>%
    filter(nrow(data) == 24, sensor_id %in% sts_sensors_vec) %>%
    ungroup() %>%
    # slice_sample(n = 20) %>%
    arrange(sensor_id, date_day) %>%
    group_by(sensor_id) %>%
    mutate(
      filename = glue("images/{sensor_id}/pp_{row_number() + 1000}.png")
    ) %>%
    rowwise()
}
...
dc_sts_p <- mean_tbl %>%
  ggplot(aes(x = reorder(x = sensor_id, PM2.5), y = PM2.5, fill = sts)) +
  geom_col(alpha = 0.7) +
  coord_flip() +
  labs(
    title = quickText(
      "Do the StS sensors show higher PM2.5 levels than other Bristol sensors?"
    ),
    subtitle = "Data capture better than 85% (only 4 sensors met this)",
    fill = "Sensor:",
    x = "Sensor ID",
    y = quickText(glue(
      "Average PM2.5 for period {my_format_fnc(date_on)} to {my_format_fnc(date_off)}"
    ))
  ) +
...
exc_sts_p <- exc_tbl %>%
  ggplot(aes(
    x = reorder(x = sensor_id, count_exc15),
    y = count_exc15,
    fill = sts
  )) +
  geom_col(alpha = 0.7) +
  ...
  labs(
    title = quickText(
      "Do the StS sensors show more exceedences than other Bristol sensors?"
    ),
    subtitle = "Data capture better than 85% (only 4 sensors met this)",
    fill = "Sensor:",
    x = "Sensor ID",
    y = quickText(glue(
      "Number of exceedences of WHO PM2.5 guideline for period {my_format_fnc(date_on)} to {my_format_fnc(date_off)}"
    ))
  ) +
...
ggsave(
  "plots/exceedences_STS_barplot.png",
  plot = exc_sts_p,
  scale = 1,
  # width = 700,
  # height = 400,
...
ggsave(
  "plots/datacapture_STS_barplot.png",
  plot = dc_sts_p,
  scale = 1,
  # width = 700,
  # height = 400,
...
title <- expression(paste(
  expression("PM"[2.5] * " " * mu * "g m"^-3 * "at sensor "),
  sensor_id
))
...
p <- ld_time_tbl %>%
  ...
  labs(
    title = glue("Heatmap for Sensor: {sensor_id}"),
    x = "Day",
    y = "Hour Commencing"
  ) +
  theme(legend.position = "bottom") +
