...
distance_correct <- function(dt_table_dtdes_tbl, year = 2023) {
...
extract_stat_bracket <- function(stat_bracket, s_or_b = "stat") {
...
process_year_concs <- function(cs_table) {
  cs_table |>
    mutate(
      across(
        starts_with("x2"),
        ~ map_dbl(.x, ~ map_dbl(.x, ~ extract_stat_bracket(.x, "stat")))
      ),
